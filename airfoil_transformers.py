...
class AirfoilGenerator:
    ...
    def evaluate_model(self, dataloader):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                loss = self.vae_loss_function(recon_batch, batch, mu, logvar, beta=0.5)
    ...
    def generate_airfoil(self, z=None, temperature=0.5):
    ...
    def generate_diverse_airfoils(self, num_airfoils=100, diversity_method='mixed'):
        ...
        if diversity_method == 'temperature':
            ...
            temps = np.linspace(0.8, 1.8, methods_per_airfoil)
            for temp in temps:
                airfoil = self.generate_airfoil(temperature=temp)
                airfoils.append(airfoil)
            ...
            for _ in range(methods_per_airfoil):
                z = torch.randn(self.latent_dim)
                z = z / torch.norm(z) * np.random.uniform(1.0, 2.5)
                airfoil = self.generate_airfoil(z.numpy())
                airfoils.append(airfoil)
            ...
            for _ in range(remaining):
                z = np.random.randn(self.latent_dim) * 1.5  # Higher std
                airfoil = self.generate_airfoil(z)
                airfoils.append(airfoil)
...
def main():
    ...
    try:
        ...
        # Generate and validate new airfoils with diversity methods
        ...
        for method in methods:
            ...
            diverse_airfoils = generator.generate_diverse_airfoils(num_airfoils=100, diversity_method=method)
            all_generated.extend(diverse_airfoils)
            
            # Quick diversity check for this method
        ...
        # Select best diverse set (highest internal diversity)
        best_airfoils = []
        best_diversity = 0
        
        # Test mixed method with different parameters
        for temp_range in [(0.5, 2.0), (0.8, 2.5), (1.0, 3.0)]:
            test_airfoils = []
            temps = np.linspace(temp_range[0], temp_range[1], 4)
            
            # Temperature variety
            for temp in temps:
                airfoil = generator.generate_airfoil(temperature=temp)
                test_airfoils.append(airfoil)
            
            # Spherical sampling
            for _ in range(4):
                z = torch.randn(generator.latent_dim)
                z = z / torch.norm(z) * np.random.uniform(1.5, 3.0)
                airfoil = generator.generate_airfoil(z.numpy())
                test_airfoils.append(airfoil)
            
            # High variance sampling
            for _ in range(4):
                z = np.random.randn(generator.latent_dim) * 2.0
                airfoil = generator.generate_airfoil(z)
                test_airfoils.append(airfoil)
            
            # Calculate diversity
            test_distances = generator.calculate_variance_matrix(test_airfoils)
            test_diversity = np.mean(test_distances[np.triu_indices_from(test_distances, k=1)])
            
            if test_diversity > best_diversity:
                best_diversity = test_diversity
                best_airfoils = test_airfoils
        
        print(f"\nBest diversity achieved: {best_diversity:.4f}")
        ...
        for i, airfoil in enumerate(valid_airfoils):
            filename = f'generated_airfoils/vae_airfoil_{i+1}.dat'
